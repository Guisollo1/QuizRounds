...
const POLL_CONNECTED=4000,POLL_FALLBACK=1500,MAX_IMPORT=500,REMOTE_DEVICE_KEY='quizRemoteDeviceToken:v1',CONTROLLER_DEVICE_KEY='quizAdminControllerDevice:v1';
...
function setQuestionPane(name,{scroll=false}={}){const panel=$('#adminPanelQuestions');if(!panel)return;const pane=['edit','bank','import'].includes(name)?name:'edit';panel.dataset.questionPane=pane;[...panel.querySelectorAll('.question-subtabs [data-question-pane]')].forEach(btn=>{const on=btn.dataset.questionPane===pane;btn.classList.toggle('active',on);btn.setAttribute('aria-selected',String(on));});try{localStorage.setItem('quizQuestionPane',pane);}catch{}if(scroll)panel.querySelector('.question-subtabs')?.scrollIntoView({behavior:'smooth',block:'nearest'});}
function initQuestionSubtabs(){const nav=document.querySelector('.question-subtabs');if(!nav||nav.dataset.ready)return;nav.dataset.ready='1';let pane='edit';try{pane=localStorage.getItem('quizQuestionPane')||localStorage.getItem('quizMobileQuestionPane')||'edit';}catch{}[...nav.querySelectorAll('[data-question-pane]')].forEach(btn=>btn.addEventListener('click',()=>setQuestionPane(btn.dataset.questionPane,{scroll:true})));setQuestionPane(pane);}
function setConfigPane(name,{scroll=false}={}){const panel=$('#adminPanelConfig');if(!panel)return;const pane=['room','rules','rounds'].includes(name)?name:'room';panel.dataset.configPane=pane;[...panel.querySelectorAll('.config-subtabs [data-config-pane]')].forEach(btn=>{if(!btn.matches('button'))return;const on=btn.dataset.configPane===pane;btn.classList.toggle('active',on);btn.setAttribute('aria-selected',String(on));});try{localStorage.setItem('quizConfigPane',pane);}catch{}if(scroll)panel.querySelector('.config-subtabs')?.scrollIntoView({behavior:'smooth',block:'nearest'});}
function initConfigSubtabs(){const nav=document.querySelector('.config-subtabs');if(!nav||nav.dataset.ready)return;nav.dataset.ready='1';let pane='room';try{pane=localStorage.getItem('quizConfigPane')||'room';}catch{}[...nav.querySelectorAll('[data-config-pane]')].forEach(btn=>btn.addEventListener('click',()=>setConfigPane(btn.dataset.configPane,{scroll:true})));setConfigPane(pane);}
...
async function withAction(label,fn){if(actionBusy)return false;if(room&&!remoteModeActive&&!controllerGranted){alert('Este painel está em modo de acompanhamento. Assuma o controle principal antes de enviar comandos.');return false;}actionBusy=true;const busy=$$('#contextPrimaryBtn,#remotePrimaryBtn,#startQuizBtn,#prepareRoundBtn,#openRoundBtn,#closeRoundBtn');busy.forEach(b=>{b.dataset.busyText=b.textContent;b.textContent='PROCESSANDO…';});$$('[data-admin-action],[data-remote-action],#startQuizBtn,#prepareRoundBtn,#openRoundBtn,#closeRoundBtn,#restartQuizBtn,#shuffleQueueBtn,#createRoomBtn,#pauseQuizBtn,#extend5Btn,#extend10Btn,#annulRoundBtn,#regradeRoundBtn').forEach(b=>b.disabled=true);if($('#adminActionStatus'))$('#adminActionStatus').textContent=label;diag('action',label);let ok=false;try{await fn();ok=true;diag('ok',`${label} confirmado pelo servidor`);remoteVibrate(35);}catch(e){diag('error',`${label} falhou`,{error:e.message||String(e)});alert(e.message||String(e));}finally{busy.forEach(b=>{if(b.dataset.busyText){b.textContent=b.dataset.busyText;delete b.dataset.busyText;}});actionBusy=false;if($('#adminActionStatus'))$('#adminActionStatus').textContent='';updateControls();renderRemoteControl();}return ok;}
...
async function claimRemotePairing(){
  ...
  remoteAuthorized=!!data?.authorized;remoteDeviceLabel=data?.device_label||defaultRemoteDeviceName();status.textContent='Celular autorizado com sucesso.';status.className='success';await refreshRemoteDeviceCount();
...
function remotePrimaryAction(){const p=phase(),stage=roomState?.room?.reveal_stage||'hidden',used=Number(roomState?.used_rounds||0),target=Number(roomState?.room?.planned_rounds||0);if(p==='lobby')return startQuiz();if(p==='preparing')return openPreparedRound();if(p==='question_open')return closeRound();if(p==='paused')return pauseQuiz();if(p==='result'&&stage==='hidden')return reveal('answer');if(p==='result'&&stage==='answer')return reveal('distribution');if(p==='result'&&stage==='distribution')return reveal('ranking');if(p==='result'&&stage==='ranking')return used>=target?reveal('final'):prepareRound();}
...
function renderRemoteControl(){
  updateRemoteAvailability();if(!remoteModeActive)return;const rp=phase();if(lastRemotePhase&&lastRemotePhase!==rp)remoteVibrate(rp==='question_open'?[45,35,45]:rp==='result'?[70]:35);lastRemotePhase=rp;
...
async function showAdmin(){$('#loginView').classList.add('hidden');$('#adminView').classList.remove('hidden');$('#logoutBtn').classList.remove('hidden');$('#eventModeBtn')?.classList.remove('hidden');initAdminTabs();initQuestionSubtabs();initConfigSubtabs();initEventMode();startTimerLoop();await Promise.all([loadQuestions(),loadTemplates()]);await resumeLatestRoom();if(room&&!wantsRemoteFromUrl())await claimController(false);await initRemoteFeature();await refreshRemoteDeviceCount();renderEventHealth();}
...
async function exportQuestionBank(filename){
  ...
  const header=['# QuizRounds v3.34 — banco completo de perguntas.','# Inclui respostas corretas, pontuação, tempo e metadados.','# Este arquivo pode ser importado novamente em modo ADICIONAR ou SUBSTITUIR.',''].join('\n');
...
function startTimerLoop(){clearInterval(timerTick);timerTick=setInterval(()=>{renderTimer();renderRemoteTimer();renderContextAction();},100);}
...
async function subscribe(){
  ...
  const main=db.channel(`quiz:${room.id}`,{config:{private:true,presence:{key:`admin-${controllerDeviceToken()}`}}})
    ...
    .on('presence',{event:'sync'},()=>{if(epoch!==subscriptionEpoch||main!==channel)return;const ps=Object.values(main.presenceState()).flat();presenceOnline=ps.filter(x=>x?.kind==='participant'||x?.participant_id).length;displayOnline=ps.some(x=>x?.kind==='display'||x?.display_id);presenceVersions={players:ps.filter(x=>x?.kind==='participant'||x?.participant_id).map(x=>x?.version||'antiga'),displays:ps.filter(x=>x?.kind==='display'||x?.display_id).map(x=>x?.version||'antiga'),admins:ps.filter(x=>x?.kind==='admin').map(x=>x?.version||'antiga')};versionMismatch=[...presenceVersions.players,...presenceVersions.displays,...presenceVersions.admins].some(v=>String(v)!==APP_VERSION);if(versionMismatch&&lastVersionBroadcast!==APP_VERSION){lastVersionBroadcast=APP_VERSION;main.send({type:'broadcast',event:'version_required',payload:{version:APP_VERSION}}).catch(()=>{});}if(!versionMismatch)lastVersionBroadcast='';updateLiveStats(roomState);renderRemoteControl();renderEventHealth();});
  channel=main;
  main.subscribe(async status=>{
    if(epoch!==subscriptionEpoch||main!==channel)return;
    if(status==='SUBSCRIBED'){
      clearTimeout(reconnectTimer);reconnectTimer=null;reconnectAttempt=0;conn('connected');setPoll(POLL_CONNECTED);try{await main.track({kind:'admin',version:APP_VERSION,device_label:controllerDeviceLabel(),at:Date.now()});}catch{}await refreshState(true);return;
    }
    if(['CHANNEL_ERROR','TIMED_OUT'].includes(status))scheduleReconnect(epoch);
...
function initEventMode(){try{eventModeActive=localStorage.getItem('quizEventMode:v1')==='1';}catch{}applyEventMode();}
...
function toggleEventMode(){eventModeActive=!eventModeActive;try{localStorage.setItem('quizEventMode:v1',eventModeActive?'1':'0');}catch{}applyEventMode();}
...
function renderEventHealth(){
  ...
  setHealthTile($('#healthPlayers'),players>0?'ready':'warn',`${players} conectado${players===1?'':'s'}`,`${Number(roomState?.participant_count||0)} cadastrado(s)`);
...
async function runPreflight({silent=false}={}){
  ...
  let rpcOk=false,lat=0;try{const t0=performance.now();const{data,error}=await db.rpc('admin_get_room_state',{p_room_id:room.id});lat=Math.round(performance.now()-t0);rpcOk=!error&&!!data;if(rpcOk){roomState=data;lastLatency=lat;}}catch{}
  const realtimeOk=lastConnectionKey.startsWith('connected'),displayOk=!!displayOnline,questionsOk=Number(roomState?.queued_count??queueRows.filter(q=>q.status==='queued').length)>0,controllerOk=controllerGranted||remoteModeActive,versionsOk=!versionMismatch,issues=[];
  if(!rpcOk)issues.push('Supabase');if(!realtimeOk)issues.push('Realtime');if(!displayOk)issues.push('telão');if(!questionsOk)issues.push('perguntas');if(!controllerOk)issues.push('controlador');if(!versionsOk)issues.push('versões diferentes');
  const hard=rpcOk&&realtimeOk&&questionsOk&&controllerOk&&versionsOk,all=hard&&displayOk;lastPreflight={all,hard,issues,latency:lat};renderEventHealth();
...
function renderContextAction(){
  ...
  else if(p==='question_open'){label='ENCERRAR RESPOSTAS E PONTUAR';text=`${Number(roomState?.answer_count||0)} de ${Number(roomState?.participant_count||0)} responderam.`;enabled=!!r?.status;}
  ...
  else if(p==='result'&&stage==='ranking'){label=used>=target?'MOSTRAR RESULTADO FINAL':queued>0?'PRÓXIMO ROUND':'SEM PRÓXIMA PERGUNTA';text=used>=target?'Mostra campeão e classificação final no telão.':queued>0?'Prepara a próxima pergunta com contagem sincronizada.':'Não há outra pergunta disponível.';enabled=used>=target||queued>0;}
  else if(p==='result'){label='MOSTRAR RESPOSTA';text='Inicia a sequência de revelação do resultado.';enabled=true;}
  else if(p==='finished'){label='QUIZ ENCERRADO';text='Use as opções abaixo para nova partida ou nova sala.';}
...
function newRoomAfterGame(){switchAdminTab('config');eventModeActive=false;try{localStorage.setItem('quizEventMode:v1','0');}catch{}applyEventMode();$('#roomTitle')?.focus();$('#createRoomBtn')?.scrollIntoView({behavior:'smooth',block:'center'});}
