import{avatarMarkup,normalizeAvatarKey}from'./avatars-v3.38.js';
...
export function createAvatarCity(host,badge,{maxActors=120}={}){
  ...
  function makeActor(row,index){
    ...
    const actor={id,el,seed,x:4+(seed%88),lane,scale:actorScale(seed,lane),nextAt:performance.now()+((seed>>>12)%2800),avatarKey:'',connected:true,ready:false};
  ...
  function moveTo(actor,x,lane,duration,direction){actor.x=Math.max(2,Math.min(94,x));applyDepth(actor,lane);actor.el.style.setProperty('--city-move-time',`${Math.max(.35,duration).toFixed(2)}s`);actor.el.style.setProperty('--city-x',`${actor.x.toFixed(2)}vw`);setActorClass(actor,'walking',direction);}
  ...
  function walk(actor,now){clearExtras(actor);const target=between(actor,3,93),delta=target-actor.x,duration=between(actor,density==='high'?4.5:2.7,density==='high'?9:6.8);moveTo(actor,target,Math.floor(between(actor,0,5)),duration,delta<0?'left':'right');actor.nextAt=now+duration*1000+between(actor,650,2200);}
  ...
  function setRoster(rows=[]){
    ...
    list.forEach((row,index)=>{
      ...
      actor.connected=connected;actor.ready=!!row.ready;actor.el.classList.toggle('offline',!connected);actor.el.classList.toggle('is-ready',actor.ready);actor.el.classList.toggle('city-leader',!!medal);actor.el.querySelector('.city-rank-crown').textContent=medal;actor.el.querySelector('.city-name').textContent=String(row.display_name||'Jogador');setAvatar(actor,row.avatar_key||'robot');applyDepth(actor,actor.lane);
