import{db,$,esc,renderRank,setConnection,serverRemaining,sleep}from'./common-v3.9.js';

let room=null,roomState=null,queueRows=[],questionBank=[],participants=[],historyRows=[],channel=null,progressChannel=null,poll=null,serverOffset=0,timerTick=null,reconnectTimer=null,reconnectAttempt=0,auditTick=null,actionBusy=false,presenceOnline=null,progressRefreshTimer=null,lastAdminQr='',activeAdminTab='questions',autoOpening=false,autoClosing=false,lastLatency=0,subscriptionEpoch=0,lastConnectionKey='',refreshSeq=0,refreshAppliedSeq=0;
...
const POLL_CONNECTED=4000,POLL_FALLBACK=1500,MAX_IMPORT=500;
...
async function withAction(label,fn){if(actionBusy)return;actionBusy=true;$$('[data-admin-action],#startQuizBtn,#prepareRoundBtn,#openRoundBtn,#closeRoundBtn,#restartQuizBtn,#shuffleQueueBtn,#createRoomBtn,#pauseQuizBtn,#extend5Btn,#extend10Btn,#annulRoundBtn,#regradeRoundBtn').forEach(b=>b.disabled=true);if($('#adminActionStatus'))$('#adminActionStatus').textContent=label;try{await fn();}catch(e){alert(e.message||String(e));}finally{actionBusy=false;if($('#adminActionStatus'))$('#adminActionStatus').textContent='';updateControls();}}
...
async function showAdmin(){$('#loginView').classList.add('hidden');$('#adminView').classList.remove('hidden');$('#logoutBtn').classList.remove('hidden');initAdminTabs();startTimerLoop();await Promise.all([loadQuestions(),loadTemplates()]);await resumeLatestRoom();}
async function resumeLatestRoom(){const{data,error}=await db.rpc('admin_get_latest_room');if(error||!data){conn(navigator.onLine?'idle':'offline');updateControls();return;}room=data;$('#plannedRounds').value=room.planned_rounds||10;$('#randomizeQueueToggle').checked=!!room.randomize_queue;$('#roomTitle').value=room.title||'Quiz ao vivo';$('#roomInfo').textContent=`Código: ${room.code}`;$('#roomProgress').classList.remove('hidden');await Promise.all([refreshState(true),loadQueue(),loadAudit(),loadParticipants(),loadHistory()]);await subscribe();}

async function createRoom(){await withAction('Criando sala…',async()=>{const{data,error}=await db.rpc('admin_create_room_v3',{p_title:$('#roomTitle').value.trim()||'Quiz ao vivo',p_planned_rounds:plannedRounds()});if(error)throw error;room=data;roomState=null;queueRows=[];presenceOnline=null;lastAdminQr='';$('#roomInfo').textContent=`Código: ${room.code}`;$('#roomProgress').classList.remove('hidden');await Promise.all([refreshState(true),loadQueue(),loadAudit(),loadParticipants()]);await saveRoomSettings(false);await subscribe();});}
...
async function saveQuestion(){const type=$('#qType').value,prompt=$('#qPrompt').value.trim();let opts=null,cc=null,cn=null;if(!prompt)return alert('Digite a pergunta.');if(type==='choice'){opts=['A','B','C','D'].map(k=>({key:k,text:$(`#opt${k}`).value.trim()}));if(opts.some(o=>!o.text))return alert('Preencha as quatro alternativas.');cc=$('#correctChoice').value;}else{cn=Number($('#correctNumber').value);if(!Number.isFinite(cn))return alert('Informe o valor correto.');}
  const{data:id,error}=await db.rpc('admin_create_question_v2',{p_prompt:prompt,p_type:type,p_options:opts,p_correct_choice:cc,p_correct_number:Number.isFinite(cn)?cn:null,p_points:Number($('#points').value)||10,p_time_limit:Number($('#timeLimit').value)||30});if(error){$('#questionMsg').textContent=error.message;return;}
  const meta=await db.rpc('admin_update_question_metadata',{p_question_id:id,p_category:$('#qCategory').value.trim()||'Geral',p_difficulty:$('#qDifficulty').value,p_score_enabled:$('#qScoreEnabled').checked,p_speed_bonus_pct:Number($('#qSpeedBonus').value)||0,p_is_tiebreaker:$('#qTiebreaker').checked,p_presenter_notes:$('#qPresenterNotes').value.trim()});if(meta.error){$('#questionMsg').textContent=`Pergunta salva, mas metadados falharam: ${meta.error.message}`;}else $('#questionMsg').textContent='Pergunta salva no banco.';
  $('#qPrompt').value='';['A','B','C','D'].forEach(k=>$(`#opt${k}`).value='');$('#correctNumber').value='';$('#qPresenterNotes').value='';await loadQuestions();}
...
function splitTxtBlocks(text){
  ...
  for(const line of lines){
    ...
    if(/^\s*(PERGUNTA|QUEST[AÃ]O|QUESTION)\s*:/i.test(line)&&current.some(x=>x.trim()))flush();
    current.push(line);
  }
  flush();return blocks;
}
...
function renderImportPreview(){
  ...
  const button=$('#importQuestionsBtn');
  button.disabled=valid===0||errors>0;
  button.textContent=mode==='replace'?'Substituir banco pelo lote':'Adicionar lote validado';
  button.classList.toggle('replace-armed',mode==='replace'&&!button.disabled);
...
async function exportQuestionBank(filename){
  ...
  const header=['# QuizRounds v3.9 — banco completo de perguntas.','# Inclui respostas corretas, pontuação, tempo e metadados.','# Este arquivo pode ser importado novamente em modo ADICIONAR ou SUBSTITUIR.',''].join('\n');
...
async function importBulkQuestions(){
  analyzeBulkText();
  if(!parsedImport.questions.length||parsedImport.errors.length)return;
  const mode=currentImportMode(),button=$('#importQuestionsBtn');
  if(mode==='replace'){
  ...
  try{
    ...
    button.textContent=modeNow==='replace'?'Substituir banco pelo lote':'Adicionar lote validado';
    button.disabled=parsedImport.questions.length===0||parsedImport.errors.length>0;
    button.classList.toggle('replace-armed',modeNow==='replace'&&!button.disabled);
  }
}
async function readTxtFile(file){
  if(!file)return;
  if(file.size>2*1024*1024)return alert('TXT máximo: 2 MB.');
  $('#bulkQuestionText').value=await file.text();analyzeBulkText();
}

function questionMatches(q){const term=($('#questionSearch')?.value||'').trim().toLowerCase(),cat=$('#questionCategoryFilter')?.value||'',dif=$('#questionDifficultyFilter')?.value||'',usage=$('#questionUsageFilter')?.value||'';if(term&&!`${q.prompt} ${q.category||''}`.toLowerCase().includes(term))return false;if(cat&&q.category!==cat)return false;if(dif&&q.difficulty!==dif)return false;if(usage==='unused'&&Number(q.use_count||0)>0)return false;if(usage==='recent'&&!q.last_used_at)return false;return true;}
...
function renderQuestionBank(){const canQueue=canEditQueue()&&!queueAtLimit(),rows=questionBank.filter(questionMatches);$('#questionList').innerHTML=rows.map(q=>{const archived=!!q.archived,queued=queueRows.some(x=>x.question_id===q.id),recentBlocked=recentQuestionIds.has(q.id)&&Number(roomSettings().avoid_recent_games||0)>0,disabled=!canQueue||archived||queued||recentBlocked;return `<div class="question-item ${archived?'archived':''}" data-id="${q.id}"><div><strong>${esc(q.prompt)}</strong><div class="question-meta">${esc(q.category||'Geral')} • ${esc(q.difficulty||'medio')} • ${q.question_type==='choice'?'Escolha':'Número'} • ${q.score_enabled===false?'SEM PONTOS':`${q.points} pts`} • ${q.time_limit_seconds}s${q.speed_bonus_pct?` • +${q.speed_bonus_pct}% velocidade`:''}${q.is_tiebreaker?' • DESEMPATE':''}${q.use_count?` • usada ${q.use_count}x`:''}${recentBlocked?' • RECENTE':''}</div></div><div class="item-actions"><button data-act="queue" ${disabled?'disabled':''}>${queued?'Já escolhida':archived?'Arquivada':recentBlocked?'Usada recentemente':'Adicionar'}</button><button class="ghost" data-act="duplicate">Duplicar</button><button class="ghost" data-act="archive">${archived?'Restaurar':'Arquivar'}</button></div></div>`;}).join('')||'<div class="empty">Nenhuma pergunta neste filtro.</div>';$$('#questionList .question-item').forEach(el=>{const q=questionBank.find(x=>x.id===el.dataset.id);el.querySelector('[data-act="queue"]').addEventListener('click',()=>queueQuestion(el.dataset.id));el.querySelector('[data-act="duplicate"]').addEventListener('click',()=>duplicateQuestion(el.dataset.id));el.querySelector('[data-act="archive"]').addEventListener('click',()=>setArchived(el.dataset.id,!q?.archived));});}
...
async function saveRoomSettings(show=true){if(!room)return show&&alert('Crie a sala primeiro.');const{data,error}=await db.rpc('admin_update_room_settings',{p_room_id:room.id,p_patch:getSettingsPatch()});if(error)return show&&alert(error.message);room=data;await refreshState(true);await loadRecentQuestionIds();renderQuestionBank();if(show)alert('Configurações salvas.');}
...
async function useTemplate(){const id=$('#templateSelect').value;if(!id)return alert('Selecione um modelo.');await withAction('Criando sala do modelo…',async()=>{const{data,error}=await db.rpc('admin_create_from_template',{p_template_id:id,p_title:null});if(error)throw error;room=data;roomState=null;queueRows=[];presenceOnline=null;lastAdminQr='';$('#plannedRounds').value=room.planned_rounds||10;await Promise.all([refreshState(true),loadQueue(),loadAudit(),loadParticipants(),loadQuestions()]);await subscribe();switchAdminTab('config');});}
async function duplicateRoom(){if(!room)return alert('Crie ou abra uma sala.');const title=prompt('Nome da cópia:',`${room.title} — cópia`);if(!title)return;await withAction('Duplicando sala…',async()=>{const{data,error}=await db.rpc('admin_duplicate_room',{p_room_id:room.id,p_title:title});if(error)throw error;room=data;roomState=null;queueRows=[];presenceOnline=null;lastAdminQr='';$('#plannedRounds').value=room.planned_rounds||10;await Promise.all([refreshState(true),loadQueue(),loadAudit(),loadParticipants(),loadQuestions()]);await subscribe();switchAdminTab('config');});}

async function startQuiz(){if(!room)return;const target=Number(roomState?.room?.planned_rounds||room.planned_rounds||plannedRounds()),joined=Number(roomState?.participant_count||0);if(joined<1)return alert('Entre com pelo menos 1 participante no lobby antes de começar.');if(queueRows.length!==target)return alert(`Escolha exatamente ${target} perguntas.`);await saveRoomSettings(false);await withAction('Preparando primeiro round…',async()=>{const{error}=await db.rpc('admin_start_quiz',{p_room_id:room.id});if(error)throw error;await Promise.all([refreshState(true),loadQueue(),loadAudit()]);});}
...
async function refreshState(force=false){if(!room)return false;const seq=++refreshSeq,t=performance.now();const{data,error}=await db.rpc('admin_get_room_state',{p_room_id:room.id});lastLatency=Math.round(performance.now()-t);if($('#latencyBadge')){$('#latencyBadge').textContent=`Latência ${lastLatency} ms`;$('#latencyBadge').className=`badge ${lastLatency<400?'ready':lastLatency<1200?'neutral':'warn'}`;}if(error){if(force&&seq>=refreshAppliedSeq)$('#roundHint').textContent=`Falha ao sincronizar: ${error.message}`;return false;}if(seq<refreshAppliedSeq)return false;refreshAppliedSeq=seq;roomState=data;if(data.room)room={...room,...data.room};serverOffset=Date.parse(data.server_now)-Date.now();$('#randomizeQueueToggle').checked=!!room.randomize_queue;renderRank($('#adminRanking'),data.ranking||[]);updateLiveStats(data);$('#stateVersionAdmin').textContent=`v${data.room?.state_version||0} • g${data.room?.generation||1}`;$('#currentQuestionAdmin').textContent=data.round?.prompt||data.prepared?.prompt||'Nenhuma pergunta em execução.';$('#roomProgress').textContent=`${data.used_rounds||0}/${data.room?.planned_rounds||0} rounds • ${data.queued_count||0} aguardando • ${data.ready_count||0} prontos`;renderAdminQr();fillSettings();if($('#eventStatusBadge')&&data.room?.is_rehearsal)$('#eventStatusBadge').textContent=`${statusLabel(data.room.phase)} • ENSAIO`;renderPresenter();renderRankingMovers();updatePresentationStage();updateControls();renderTimer();await maybeAutoOpen();return true;}
...
function updateControls(){const has=!!room;if(!has){$$('#displayBtn,#fullscreenDisplayBtn,#copyJoinLinkBtn,#startQuizBtn,#prepareRoundBtn,#openRoundBtn,#closeRoundBtn,#restartQuizBtn,#shuffleQueueBtn,#pauseQuizBtn,#extend5Btn,#extend10Btn,#annulRoundBtn,#regradeRoundBtn').forEach(b=>b.disabled=true);return;}const p=phase(),target=Number(roomState?.room?.planned_rounds||plannedRounds()),used=Number(roomState?.used_rounds||0),queued=Number(roomState?.queued_count||0),r=roomState?.round;$('#eventStatusBadge').textContent=statusLabel(p);$('#roundProgressBadge').textContent=`${used} / ${target}`;$('#displayBtn').disabled=false;$('#fullscreenDisplayBtn').disabled=false;$('#copyJoinLinkBtn').disabled=false;const joined=Number(roomState?.participant_count||0);$('#startQuizBtn').disabled=actionBusy||!(p==='lobby'&&queueRows.length===target&&used===0&&joined>=1);$('#startQuizBtn').title=joined<1?'Aguarde pelo menos 1 participante entrar no lobby.':'Iniciar com os participantes atuais (sem limite máximo definido pelo painel).';$('#plannedRounds').disabled=p!=='lobby';$('#randomizeQueueToggle').disabled=p!=='lobby'||actionBusy;$('#shuffleQueueBtn').disabled=actionBusy||!(p==='lobby'&&queueRows.length>=2);$('#restartQuizBtn').disabled=actionBusy||used===0;$('#prepareRoundBtn').disabled=actionBusy||!(p==='result'&&used<target&&queued>0);$('#openRoundBtn').disabled=actionBusy||p!=='preparing';$('#closeRoundBtn').disabled=actionBusy||!(p==='question_open'&&r?.status==='open');$('#pauseQuizBtn').disabled=actionBusy||p==='question_open'||p==='finished'||p==='lobby';$('#pauseQuizBtn').textContent=p==='paused'?'Retomar partida':'Pausar partida';$('#extend5Btn').disabled=actionBusy||!(p==='question_open'&&r?.accepting_responses);$('#extend10Btn').disabled=$('#extend5Btn').disabled;$('#annulRoundBtn').disabled=actionBusy||!(['result','finished'].includes(p)&&r?.status==='closed'&&!r.annulled);$('#regradeRoundBtn').disabled=actionBusy||!(['result','finished'].includes(p)&&r?.status==='closed');$$('[data-reveal]').forEach(b=>b.disabled=actionBusy||!(['result','finished'].includes(p)));$('#createRoomBtn').disabled=actionBusy;$('#createRoomBtn').textContent=room?'Criar outra sala':'Criar sala';}
...
function startTimerLoop(){clearInterval(timerTick);timerTick=setInterval(renderTimer,100);}

async function loadParticipants(){if(!room)return;const{data,error}=await db.rpc('admin_list_participants',{p_room_id:room.id});if(error)return;participants=data||[];const now=Date.now(),fmtSeen=v=>{const ms=now-Date.parse(v||0);if(!Number.isFinite(ms)||ms<0)return 'sem heartbeat';if(ms<60000)return `visto há ${Math.max(1,Math.round(ms/1000))}s`;if(ms<3600000)return `visto há ${Math.round(ms/60000)}min`;return `visto ${new Date(v).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})}`;};$('#participantAdminList').innerHTML=participants.map(p=>`<div class="participant-admin-row ${p.kicked?'kicked':''}" data-id="${p.id}"><span class="presence-dot ${now-Date.parse(p.last_seen_at)<90000?'online':'offline'}"></span><strong>${esc(p.display_name)}</strong><span>${p.ready?'PRONTO':'AGUARDANDO'}</span><b>${Number(p.total_points)||0} pts</b><small>${Number(p.answer_count)||0} resp • ${Number(p.missed_count)||0} perd. • ${fmtSeen(p.last_seen_at)} • 🔥 ${Number(p.current_streak)||0}</small><div class="participant-admin-actions"><button class="ghost small" data-detail>Detalhes</button><button class="ghost small" data-kick ${p.kicked?'disabled':''}>Remover</button></div></div>`).join('')||'<div class="empty">Nenhum participante.</div>';$$('[data-detail]').forEach(b=>b.addEventListener('click',()=>{const p=participants.find(x=>x.id===b.closest('[data-id]').dataset.id);if(!p)return;alert(`${p.display_name}\n\nPontos: ${Number(p.total_points)||0}\nRespostas: ${Number(p.answer_count)||0}\nRounds perdidos: ${Number(p.missed_count)||0}\nStreak atual: ${Number(p.current_streak)||0}\nMelhor streak: ${Number(p.best_streak)||0}\nÚltimo heartbeat: ${p.last_seen_at?new Date(p.last_seen_at).toLocaleString('pt-BR'):'sem registro'}\nStatus: ${p.kicked?'REMOVIDO':(now-Date.parse(p.last_seen_at)<90000?'CONECTADO':'DESCONECTADO')}`);}));$$('[data-kick]').forEach(b=>b.addEventListener('click',()=>kickParticipant(b.closest('[data-id]').dataset.id)));}
...
async function subscribe(){
  ...
  const main=db.channel(`quiz:${room.id}`,{config:{private:true}})
    ...
    .on('presence',{event:'sync'},()=>{if(epoch!==subscriptionEpoch||main!==channel)return;const ps=Object.values(main.presenceState()).flat();presenceOnline=ps.filter(x=>x?.kind==='participant'||x?.participant_id).length;updateLiveStats(roomState);});
  channel=main;
...
$('#saveQuestionBtn').addEventListener('click',saveQuestion);$('#refreshQuestionsBtn').addEventListener('click',loadQuestions);$('#questionScope').addEventListener('change',loadQuestions);['#questionSearch','#questionCategoryFilter','#questionDifficultyFilter','#questionUsageFilter'].forEach(id=>$(id)?.addEventListener(id==='#questionSearch'?'input':'change',renderQuestionBank));$('#qType').addEventListener('change',()=>{const n=$('#qType').value==='numeric';$('#choiceFields').classList.toggle('hidden',n);$('#numericFields').classList.toggle('hidden',!n);});
