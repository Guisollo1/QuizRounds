...
export function avatarPickerMarkup(selected='robot'){
  ...
  const choices=AVATARS.map(a=>`<button type="button" class="avatar-choice ${a.key===current?'selected':''}" data-avatar-choice="${a.key}" aria-pressed="${a.key===current?'true':'false'}" title="${a.label}">${avatarMarkup(a.key,{animated:true})}<span>${a.label}</span></button>`).join('');
