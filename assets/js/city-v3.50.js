import{avatarMarkup,normalizeAvatarKey}from'./avatars-v3.50.js';
...
const X_BOUNDS_BY_SCENE={
  office:[{min:12,max:26},{min:16,max:34},{min:24,max:56},{min:58,max:82},{min:68,max:88}],
  laboratory:[{min:12,max:28},{min:16,max:36},{min:26,max:60},{min:58,max:84},{min:68,max:90}],
  industry:[{min:10,max:24},{min:18,max:38},{min:34,max:62},{min:60,max:82},{min:72,max:90}],
  platform:[{min:12,max:26},{min:22,max:40},{min:36,max:60},{min:56,max:78},{min:70,max:88}],
  default:[{min:10,max:24},{min:16,max:36},{min:26,max:62},{min:58,max:84},{min:68,max:90}]
};
...
export function createAvatarCity(host,badge,{maxActors=120}={}){
  ...
  function currentScene(){
    const scene=String(city?.dataset?.scene||'').toLowerCase();
    return X_BOUNDS_BY_SCENE[scene]?scene:'default';
  }
  function laneBounds(lane){
    const rows=X_BOUNDS_BY_SCENE[currentScene()]||X_BOUNDS_BY_SCENE.default;
    return rows[Math.max(0,Math.min(4,lane))]||rows[2]||{min:10,max:90};
  }
  function clampX(x,lane){
  ...
  function makeActor(row,index){
    ...
    const initialBounds=laneBounds(lane);const actor={id,el,seed,x:clampX(initialBounds.min+((seed%1000)/1000)*(initialBounds.max-initialBounds.min),lane),lane,scale:actorScale(seed,lane),nextAt:performance.now()+((seed>>>12)%2800),avatarKey:'',connected:true,ready:false,direction:'down'};
  ...
  function moveTo(actor,x,lane,duration,direction){actor.x=clampX(x,lane);actor.direction=direction;applyDepth(actor,lane);actor.el.style.setProperty('--city-move-time',`${Math.max(.35,duration).toFixed(2)}s`);actor.el.style.setProperty('--city-x',`${actor.x.toFixed(2)}%`);setActorClass(actor,'walking',direction);}
  ...
  function walk(actor,now){
    ...
    if(vertical){
      ...
      const targetX=clampX(actor.x+between(actor,-4,4),targetLane);
      moveTo(actor,targetX,targetLane,duration,direction);
    }else{
      const bounds=laneBounds(actor.lane),target=between(actor,bounds.min,bounds.max),delta=target-actor.x;
      moveTo(actor,target,actor.lane,duration,delta<0?'left':'right');
    }
    actor.nextAt=now+duration*1000+between(actor,650,2200);
  }
  function sideStep(actor,now){clearExtras(actor);const delta=(nextRand(actor)>.5?1:-1)*between(actor,3.5,10),target=clampX(actor.x+delta,actor.lane),duration=between(actor,.65,1.35);actor.x=target;actor.direction=delta<0?'left':'right';actor.el.style.setProperty('--city-move-time',`${duration.toFixed(2)}s`);actor.el.style.setProperty('--city-x',`${target.toFixed(2)}%`);setActorClass(actor,'sidestepping',actor.direction);actor.nextAt=now+duration*1000+between(actor,500,1500);}
  ...
  function pairInteraction(a,b,now,type){
    ...
    const lane=Math.floor(nextRand(a)*4),bounds=laneBounds(lane),gap=phaseIsCompact(phase)?4.5:3.2,duration=1.1+nextRand(a)*1.2,center=Math.max(bounds.min+gap,Math.min(bounds.max-gap,bounds.min+nextRand(a)*(bounds.max-bounds.min)));
