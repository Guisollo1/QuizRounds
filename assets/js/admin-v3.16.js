import{db,$,esc,renderRank,setConnection,serverRemaining,sleep}from'./common-v3.16.js';
import{avatarMarkup}from'./avatars-v3.16.js';
...
async function withAction(label,fn){if(actionBusy)return false;actionBusy=true;$$('[data-admin-action],#startQuizBtn,#prepareRoundBtn,#openRoundBtn,#closeRoundBtn,#restartQuizBtn,#shuffleQueueBtn,#createRoomBtn,#pauseQuizBtn,#extend5Btn,#extend10Btn,#annulRoundBtn,#regradeRoundBtn').forEach(b=>b.disabled=true);if($('#adminActionStatus'))$('#adminActionStatus').textContent=label;let ok=false;try{await fn();ok=true;}catch(e){alert(e.message||String(e));}finally{actionBusy=false;if($('#adminActionStatus'))$('#adminActionStatus').textContent='';updateControls();}return ok;}
...
async function createRoom(){if(room){const risk=roomRiskSummary();if(risk.hasWork){const msg=`A sala atual ${room.code} tem ${risk.joined} participante(s) e ${risk.queued} pergunta(s) na fila. Criar outra sala NÃO transfere jogadores nem perguntas.\n\nDeseja realmente criar outra sala?`;if(!confirm(msg))return;}}await withAction('Criando sala…',async()=>{const{data,error}=await db.rpc('admin_create_room_v3',{p_title:$('#roomTitle').value.trim()||'Quiz ao vivo',p_planned_rounds:plannedRounds()});if(error)throw error;room=data;rememberRoom();roomState=null;queueRows=[];presenceOnline=null;lastAdminQr='';$('#roomInfo').textContent=`Código: ${room.code}`;$('#roomInfo').classList.add('room-protected');$('#roomProgress').classList.remove('hidden');await Promise.all([refreshState(true),loadQueue(),loadAudit(),loadParticipants()]);await saveRoomSettings(false);await subscribe();});}
...
async function exportQuestionBank(filename){
  ...
  const header=['# QuizRounds v3.16 — banco completo de perguntas.','# Inclui respostas corretas, pontuação, tempo e metadados.','# Este arquivo pode ser importado novamente em modo ADICIONAR ou SUBSTITUIR.',''].join('\n');
...
async function useTemplate(){const id=$('#templateSelect').value;if(!id)return alert('Selecione um modelo.');await withAction('Criando sala do modelo…',async()=>{const{data,error}=await db.rpc('admin_create_from_template',{p_template_id:id,p_title:null});if(error)throw error;room=data;rememberRoom();roomState=null;queueRows=[];presenceOnline=null;lastAdminQr='';$('#plannedRounds').value=room.planned_rounds||10;await Promise.all([refreshState(true),loadQueue(),loadAudit(),loadParticipants(),loadQuestions()]);await subscribe();switchAdminTab('config');});}
async function duplicateRoom(){if(!room)return alert('Crie ou abra uma sala.');const title=prompt('Nome da cópia:',`${room.title} — cópia`);if(!title)return;await withAction('Duplicando sala…',async()=>{const{data,error}=await db.rpc('admin_duplicate_room',{p_room_id:room.id,p_title:title});if(error)throw error;room=data;rememberRoom();roomState=null;queueRows=[];presenceOnline=null;lastAdminQr='';$('#plannedRounds').value=room.planned_rounds||10;await Promise.all([refreshState(true),loadQueue(),loadAudit(),loadParticipants(),loadQuestions()]);await subscribe();switchAdminTab('config');});}

async function startQuiz(){if(!room)return alert('Crie ou abra uma sala primeiro.');if(phase()!=='lobby')return alert('O quiz só pode ser iniciado enquanto a sala está no lobby.');await Promise.all([refreshState(true),loadQueue()]);let target=Number(roomState?.room?.planned_rounds||room.planned_rounds||plannedRounds()),joined=Number(roomState?.participant_count||0);if(joined<1)return alert('Entre com pelo menos 1 participante no lobby antes de começar.');if(queueRows.length!==target){if(queueRows.length===0){const recover=confirm(`A sala ${room.code} tem ${joined} participante(s), mas está sem perguntas. Isso pode acontecer quando outra sala foi criada depois de montar a fila.\n\nQuer tentar recuperar automaticamente uma fila completa de ${target} perguntas de uma sala anterior compatível?`);if(recover){const ok=await withAction('Recuperando fila anterior…',async()=>{const{data,error}=await db.rpc('admin_recover_queue_for_room',{p_room_id:room.id});if(error)throw error;const recovered=Number(data?.recovered||0);if(recovered<1)throw new Error('Não encontrei uma sala anterior compatível com uma fila completa para recuperar.');await Promise.all([loadQueue(),refreshState(true),loadAudit()]);});if(!ok)return;target=Number(roomState?.room?.planned_rounds||room.planned_rounds||plannedRounds());}}if(queueRows.length!==target)return alert(`A sala ${room.code} precisa de exatamente ${target} perguntas. Atualmente há ${queueRows.length}. Vá em Configuração ou no Banco de Perguntas para completar a fila.`);}const saved=await withAction('Validando configurações…',async()=>{await saveRoomSettings(false);});if(!saved)return;await withAction('Preparando primeiro round…',async()=>{const{error}=await db.rpc('admin_start_quiz',{p_room_id:room.id});if(error)throw error;await Promise.all([refreshState(true),loadQueue(),loadAudit()]);});}
