...
export function renderRank(el,rows,opts={}){
  ...
  const html=rows.map((r,i)=>{
    ...
    return `<li class="rank-row${top}${self}" data-rank="${pos}"><span class="rank-pos">${pos}</span><span class="rank-avatar">${avatarMarkup(r.avatar_key||'robot',{compact:true,animated:pos<=3})}</span><strong>${esc(r.display_name)}${move}</strong><b>${Number(r.total_points)||0} pts</b></li>`;
