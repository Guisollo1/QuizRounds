...
export function renderRank(el,rows,opts={}){
  ...
  const signature=JSON.stringify(rows.map((r,i)=>[i+1,String(r.participant_id??''),String(r.display_name??''),Number(r.total_points)||0,selfId&&String(r.participant_id)===selfId?1:0]));
