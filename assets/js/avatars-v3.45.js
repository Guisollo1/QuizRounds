...
export function avatarPickerMarkup(selected='robot'){
  ...
  const choices=AVATARS.map(a=>`<button type="button" class="avatar-choice ${a.key===current?'selected':''}" data-avatar-choice="${a.key}" aria-pressed="${a.key===current?'true':'false'}" title="${a.label}">${avatarMarkup(a.key,{animated:a.key===current})}<span>${a.label}</span></button>`).join('');
  return `<div class="avatar-picker-grid avatar-picker-grid-all" role="group" aria-label="Todos os avatares">${choices}</div>`;
}
