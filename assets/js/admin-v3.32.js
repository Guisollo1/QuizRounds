import{db,$,esc,renderRank,setConnection,serverRemaining,sleep,APP_VERSION}from'./common-v3.32.js';
import{avatarMarkup}from'./avatars-v3.32.js';
...
async function showAdmin(){$('#loginView').classList.add('hidden');$('#adminView').classList.remove('hidden');$('#logoutBtn').classList.remove('hidden');$('#eventModeBtn')?.classList.remove('hidden');initAdminTabs();initMobileQuestionNav();initEventMode();startTimerLoop();await Promise.all([loadQuestions(),loadTemplates()]);await resumeLatestRoom();if(room&&!wantsRemoteFromUrl())await claimController(false);await initRemoteFeature();await refreshRemoteDeviceCount();renderEventHealth();}
...
async function exportQuestionBank(filename){
  ...
  const header=['# QuizRounds v3.32 — banco completo de perguntas.','# Inclui respostas corretas, pontuação, tempo e metadados.','# Este arquivo pode ser importado novamente em modo ADICIONAR ou SUBSTITUIR.',''].join('\n');
