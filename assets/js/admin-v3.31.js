...
async function showAdmin(){$('#loginView').classList.add('hidden');$('#adminView').classList.remove('hidden');$('#logoutBtn').classList.remove('hidden');$('#eventModeBtn')?.classList.remove('hidden');initAdminTabs();initMobileQuestionNav();initEventMode();startTimerLoop();await Promise.all([loadQuestions(),loadTemplates()]);await resumeLatestRoom();if(room&&!wantsRemoteFromUrl())await claimController(false);await initRemoteFeature();renderEventHealth();}
...
async function exportQuestionBank(filename){
  ...
  const header=['# QuizRounds v3.31 — banco completo de perguntas.','# Inclui respostas corretas, pontuação, tempo e metadados.','# Este arquivo pode ser importado novamente em modo ADICIONAR ou SUBSTITUIR.',''].join('\n');
...
async function refreshState(force=false){if(!room)return false;const seq=++refreshSeq,t=performance.now();const{data,error}=await db.rpc('admin_get_room_state',{p_room_id:room.id});lastLatency=Math.round(performance.now()-t);if($('#latencyBadge')){$('#latencyBadge').textContent=`Latência ${lastLatency} ms`;$('#latencyBadge').className=`badge ${lastLatency<400?'ready':lastLatency<1200?'neutral':'warn'}`;}if(error){if(force&&seq>=refreshAppliedSeq)$('#roundHint').textContent=`Falha ao sincronizar: ${error.message}`;return false;}if(seq<refreshAppliedSeq)return false;refreshAppliedSeq=seq;roomState=data;if(data.room){room={...room,...data.room};rememberRoom();}serverOffset=Date.parse(data.server_now)-Date.now();$('#randomizeQueueToggle').checked=!!room.randomize_queue;renderRank($('#adminRanking'),data.ranking||[],{movers:data.round?.movers||[]});updateLiveStats(data);$('#stateVersionAdmin').textContent=`v${data.room?.state_version||0} • g${data.room?.generation||1}`;$('#currentQuestionAdmin').textContent=data.round?.prompt||data.prepared?.prompt||'Nenhuma pergunta em execução.';$('#roomProgress').textContent=`${data.used_rounds||0}/${data.room?.planned_rounds||0} rounds • ${data.queued_count||0} aguardando • ${data.ready_count||0} prontos`;renderAdminQr();fillSettings();if($('#eventStatusBadge')&&data.room?.is_rehearsal)$('#eventStatusBadge').textContent=`${statusLabel(data.room.phase)} • ENSAIO`;renderPresenter();renderRankingMovers();updatePresentationStage();updateControls();renderTimer();renderRemoteControl();renderEventHealth();await maybeAutoOpen();return true;}
...
async function subscribe(){
  ...
  const main=db.channel(`quiz:${room.id}`,{config:{private:true}})
    ...
    .on('presence',{event:'sync'},()=>{if(epoch!==subscriptionEpoch||main!==channel)return;const ps=Object.values(main.presenceState()).flat();presenceOnline=ps.filter(x=>x?.kind==='participant'||x?.participant_id).length;displayOnline=ps.some(x=>x?.kind==='display'||x?.display_id);updateLiveStats(roomState);renderRemoteControl();renderEventHealth();});
  channel=main;
...
async function runPreflight({silent=false}={}){
  ...
  const realtimeOk=lastConnectionKey.startsWith('connected'),displayOk=!!displayOnline,questionsOk=Number(roomState?.queued_count??queueRows.filter(q=>q.status==='queued').length)>0,controllerOk=controllerGranted||remoteModeActive,issues=[];
  if(!rpcOk)issues.push('Supabase');if(!realtimeOk)issues.push('Realtime');if(!displayOk)issues.push('telão');if(!questionsOk)issues.push('perguntas');if(!controllerOk)issues.push('controlador');
  const hard=rpcOk&&realtimeOk&&questionsOk&&controllerOk,all=hard&&displayOk;lastPreflight={all,hard,issues,latency:lat};renderEventHealth();
...
function openLoadSimulator(count){const u=new URL('simulator.html',location.href);u.searchParams.set('bots',String(count));u.searchParams.set('autorun','1');u.searchParams.set('from','admin');window.open(u.href,'quiz_load_simulator');}
