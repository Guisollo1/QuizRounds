...
function avatarChoicePreviewMarkup(key){
  ...
  return `<span class="avatar-choice-preview" aria-hidden="true" style="--avatar-sprite:url('${a.sprite}')"><span class="avatar-choice-preview-sprite"></span></span>`;
}
...
export function avatarMarkup(key,{animated=false,label=false,compact=false}={}){
  const a=avatarInfo(key);
  const cls=['quiz-avatar',`avatar-${a.key}`,`avatar-category-${a.category}`,`avatar-gender-${a.gender}`,animated?'avatar-animated':'',compact?'avatar-compact':''].filter(Boolean).join(' ');
  return `<span class="${cls}" data-avatar="${a.key}" style="--avatar-sprite:url('${a.sprite}');--avatar-accent:${a.accent}" aria-hidden="true"><span class="qa-shadow"></span><span class="qa-sprite"></span>${label?`<span class="qa-label">${a.label}</span>`:''}</span>`;
}
